...
def show_predict_page():
    st.title("🚦 congestion Volume Prediction")
    ...
    day = st.selectbox('Select Day of the Month', list(range(1, 32)), index=0)
    hour = st.slider('Select Hour of Day', 0, 23, 8)

    # Feature extraction
    day_name = date_to_day(year, month, day)
    day_mapping = {
    ...
    if st.checkbox("Show congestion Volume Trend Chart"):
        ...
        for h in sample_hours:
            ...
            X_h = pd.DataFrame(input_data_h)
            pred = model.predict(X_h)[0]
            sample_predictions.append(pred)

        # Plotting
        fig, ax = plt.subplots(figsize=(10,5))
        ax.plot(sample_hours, sample_predictions, marker='o', color='navy')
        ax.set_xticks(sample_hours)
        ax.set_xlabel('Hour of Day')
        ax.set_ylabel('Predicted Congestion Volume (vehicles)')
        ax.set_title('Predicted Congestion Volume Trend Over the Day')
        st.pyplot(fig)
